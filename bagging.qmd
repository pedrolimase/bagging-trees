...
\begin{center}
  ...
  Melhorando a estabilidade e acurácia \\ {\normalsize Grupo 3}} \\
...
A natureza de particionamento das árvores de decisão fornece flexibilidade e capacidade de ajuste ilimitados a elas. Isto é, com *splits* suficientes, é sempre possível obter o ajuste "perfeito". Porém isso vem a dois custos principais:
...
Bishop (2006) traz, para o caso de regressão, um argumento matemático/estatístico de que esse procedimento garantidamente reduz o erro de predição médio, embora a redução possa ser pequena.
...
Os tumores do conjunto de dados estão divididos em 63% de casos malignos e 37% de casos benignos.
...
Agora, para ilustrar a instabilidade das árvores de decisão, vamos retirar um subconjunto aleatório pequeno de 5% das observações do conjunto de treinamento e ver o que acontece com o resultado. Isso é feito duas vezes e os resultados são mostrados na @fig-rpart-tree2 a seguir.
...
set.seed(42)
train_idxs <- createDataPartition(breast_cancer$diagnosis, p = .8, 
                                  list = FALSE,
                                  times = 1)

breast_train <- breast_cancer[train_idxs, ]
breast_test <- breast_cancer[-train_idxs, ]
```

Agora podemos ajustar as àrvores com bagging usando o método `treebag` do pacote `caret`. Definimos o número de árvores no ensemble como `nbagg = 40` (outros valores na @fig-treebag-nbagg), e os parâmetros das árvores `minsplit = 2` e `cp = 0`, seguindo os resultados obtidos anteriormente. Nesse caso, a acurácia foi de 94%, um pouco maior que o árvore de decisão simples.
...
#| echo: false
#| cache: true
nbagg <- seq(10, 100, by = 10)

treebag_fits <- map(nbagg, ~ {
  train(
    diagnosis ~ .,
    data = breast_train,
    method = "treebag",
...
Note pela @fig-treebag-nbagg abaixo que, para todos os números de árvores testados, as acurácias das árvores de decisão com bagging são melhores que a acurácia da árvore de decisão simples (linha pontilhada azul) que testamos inicialmente. Vemos ainda que, nesse caso, a acurácia não muda muito com o número de árvores, e também que 40 árvores já são suficientes.

```{r}
#| label: fig-treebag-nbagg
#| fig-cap: "Acurácia de validação cruzada 10-fold da árvore de decisão com bagging em função do tamanho do ensemble (número de árvores)."
#| echo: false
qplot(nbagg, map_dbl(treebag_fits, ~ .x$results$Accuracy),
      geom = "line")+
...
A matriz de confusão a seguir mostra que, no conjunto de treinamento, todos os tumores são classificados corretamente, indicando que o voto majoritário das árvores foi correto em todos os casos.
